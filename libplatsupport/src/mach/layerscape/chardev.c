...
#define UART_DEFN(devid)                                                       \
  {                                                                            \
    .id = LS_DUART_UART##devid, .paddr = UART##devid##_PADDR, .size = BIT(12), \
    .irqs = uart##devid##_irqs, .init_fn = &uart_init                          \
  }
...
struct ps_chardevice *ps_cdev_init(enum chardev_id id, const ps_io_ops_t *o,
